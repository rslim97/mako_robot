...
def generate_launch_description():
    ...
    mako_gazebo_path = get_package_share_directory("mako_gazebo")

    world_file = LaunchConfiguration(
        "world_file",
        default=os.path.join(mako_gazebo_path, "worlds", "small_house.sdf"),
    )
    ...
    bridge_params = os.path.join(mako_gazebo_path, "config", "gz_bridge.yaml")
    ros_gz_bridge = Node(
    ...
    return LaunchDescription(
        [
            AppendEnvironmentVariable(
                ...
                value=os.path.join(mako_gazebo_path, "worlds"),
            ),
            AppendEnvironmentVariable(
                name="GZ_SIM_RESOURCE_PATH",
                value=os.path.join(mako_gazebo_path, "models"),
            ),
